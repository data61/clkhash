...
def generate_key_lists(master_secret,  # type: Union[bytes, str]
    ...
    """
    Generates two derived keys for each identifier for the master secret using a key derivation function (KDF).

    The only supported key derivation function for now is 'HKDF'.

    The previous key usage can be reproduced by setting kdf to 'legacy', but it will use the master secret twice.
    ...
    :param master_secret: a master secret (either as bytes or string)
    ...
    try:
        ...
        raise TypeError("provided 'master_secret' have to be either of type bytes or strings.")
    if kdf == 'HKDF':
        # we first create the good number of keys, and we then pack them in the expected way.
        key_tuples = hkdf(key, num_hashing_methods * num_identifier,
    ...
    if kdf == 'legacy':
        return tuple(tuple([key] * num_hashing_methods) for _ in range(num_identifier))
