...
class TestRestClientInteractionWithService(unittest.TestCase):
    ...
    def test_upload_clks(self):
        ...
        upload_response = rest_client.project_upload_clks(self.url,
                                                          p['project_id'],
                                                          p['update_tokens'][
                                                              0],
                                                          self.clk_data_1)
    ...
    def test_project_run(self):
        ...
        p_id = p['project_id']
        upload_response_1 = rest_client.project_upload_clks(self.url, p_id,
                                                            p['update_tokens'][
                                                                0],
                                                            self.clk_data_1)
        upload_response_2 = rest_client.project_upload_clks(self.url, p_id,
                                                            p['update_tokens'][
                                                                1],
                                                            self.clk_data_2)

        run_response = rest_client.run_create(self.url, p_id,
        ...
        for status_update in rest_client.watch_run_status(self.url, p_id, r_id,
                                                          p['result_token'],
                                                          10, 0.5):
        ...
        coord_result_raw = rest_client.run_get_result_text(self.url, p_id,
                                                           r_id,
                                                           p['result_token'])
        ...
        result_a = json.loads(
            rest_client.run_get_result_text(self.url, p_id, r_id,
                                            upload_response_1[
                                                'receipt_token']))
        result_b = json.loads(
            rest_client.run_get_result_text(self.url, p_id, r_id,
    ...
    def test_project_run_before_data(self):
        ...
        run_response = rest_client.run_create(self.url, p_id,
                                              ...
                                              name='clkhash rest client test')
        with pytest.raises(ServiceError):
            json.loads(rest_client.run_get_result_text(self.url, p_id,
                                                       run_response['run_id'],
                                                       upload_response_1[
                                                           'receipt_token']))
...
def test_watch_run_time_out(requests_mock):
    ...
    with pytest.raises(TimeoutError):
        for update in rest_client.watch_run_status('http://testing-es-url',
                                                   'pid', 'rid', 'apikey',
                                                   timeout=0.5,
                                                   update_period=0.01):
...
def test_watch_run_rate_limited(requests_mock):
    ...
    for update in rest_client.watch_run_status('http://testing-es-url', 'pid',
